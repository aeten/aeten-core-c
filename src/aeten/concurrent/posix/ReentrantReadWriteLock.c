...
void _new(ReentrantReadWriteLock* self) {
...
void _finalize(ReentrantReadWriteLock* self) {
	Lock_delete(self->_read_lock);
	Lock_delete(self->_write_lock);
	pthread_rwlock_destroy(&self->_rw_lock);
}

Lock* _readLock(ReentrantReadWriteLock* self) {
	return self->_read_lock;
}

Lock* _writeLock(ReentrantReadWriteLock* self) {
