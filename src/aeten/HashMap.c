...
void _new(HashMap* self, size_t key_size, size_t value_size) {
...
static void *_get_hashed(HashMap *self, void *key, uint64_t hash, int *index) {
...
void *_get(HashMap *self, void *key) {
	...
	return _get_hashed(self, key, hash, NULL);
}

void *_put(HashMap *self, void* key, void *value) {
	int idx, idx_subarray;
	uint64_t hash = isObject(key)? Object_hashCode((Object*)key): Hash_hashBuffer((char*)key, self->_key_size, self->_bits);
	void *old_value = _get_hashed(self, key, hash, &idx_subarray);
	...
	if (old_value == NULL) {
		if (hash > self->_hash_max) {
			...
			check(self->_hash_array, HeadAllocationException, "realloc(%u)", size * sizeof(List*));
		...
		if (hash < self->_hash_min) {
			...
			void     *old_array = self->_hash_array;
			uint64_t  old_min   = self->_hash_min;
			self->_hash_min     = hash;
		...
		if (self->_hash_array == NULL) {
			...
			check(self->_hash_array, HeadAllocationException, "malloc(%u)", sizeof(List*));
		}
		if (self->_hash_array[idx] == NULL) {
...
void* _remove(HashMap* self, void* key) {
	...
	int idx_subarray;
	int idx = hash - self->_hash_min;
	void *value = _get_hashed(self, key, hash, &idx_subarray);
	if (value != NULL) {
